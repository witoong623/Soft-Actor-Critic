...
class World(object):
    ...
    def restart(self):
        ...
        if self.player is not None:
            spawn_point_transform = self.player.get_transform()
            spawn_point_transform.location.z += 2.0
            spawn_point_transform.rotation.roll = 0.0
            spawn_point_transform.rotation.pitch = 0.0
            self.destroy()
            self.player = self.world.try_spawn_actor(blueprint, spawn_point_transform)
            self.show_vehicle_telemetry = False
...
class KeyboardControl(object):
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        if not self._autopilot_enabled:
            ...
            world.current_action = (self._control.throttle, self._control.steer, self._control.brake)
    ...
    def __init__(self, parent_actor, hud, gamma_correction, saving_queue):
        ...
        if not self._parent.type_id.startswith("walker.pedestrian"):
            self._camera_transforms = [
                ...
                (carla.Transform(carla.Location(x=0.8, z=1.7)), Attachment.Rigid)]
        else:
            self._camera_transforms = [
...
def game_loop(args):
    ...
    try:
        ...
        sim_world = client.load_world('ait_v4')
        if args.sync:
